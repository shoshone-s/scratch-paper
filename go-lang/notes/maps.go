...
var bookings_slice = make([]map[string]string, 0)
...
func bookTicket(userTickets int, firstName string, lastName string, email string) {
	...
	// - need to define the types for the key value pairs
	var userData = make(map[string]string)
	// adding data key value pairs to the map
	userData["firstName"] = firstName
	userData["lastName"] = lastName
	userData["email"] = email
	// in order to get the userTickets we need to convert it from uint to string
