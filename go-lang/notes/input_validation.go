...
func main() {
	...
	for {
		...
		// input validation; checking if user input a valid location for the conference
		// isValidCity := city == "Singapore" || city == "London"
		// input validation; checking if user did not enter a specific value
		// isInvalidCity := city != "Singapore" || city != "London"
		// !isValidCity <-- this is alternate syntax of the validation above
		...
		if isValidName && isValidEmail && isValidTicketNumber {
			...
			// conditionals to give better feedback on user input errors
			if !isValidName {
				if len(firstName) < 2 {
